...
[assembly: SiliconStudio.Core.Serialization.Serializers.AssemblySerializerFactory(Type = typeof(SiliconStudio.DataSerializers.RetrogradeSerializerFactory))]
...
namespace SiliconStudio.DataSerializers
{
    // Assembly attributes that defines supported serializer (only generics one so that other assemblies can do generic instantiations by themselves)
   	public static class RetrogradeSerializerFactory
	{
	    [SiliconStudio.Core.ModuleInitializer]
        internal static void Initialize()
        {
			var assemblySerializers = new AssemblySerializers(typeof(RetrogradeSerializerFactory).GetTypeInfo().Assembly);
			...
			assemblySerializers.Modules.Add(typeof(SiliconStudio.DataSerializers.Retrograde_GameSerializerFactory).GetTypeInfo().Module);
            ...
            AssemblyRegistry.Register(typeof(RetrogradeSerializerFactory).GetTypeInfo().Assembly, AssemblyCommonCategories.Engine);
        }
	}
}
